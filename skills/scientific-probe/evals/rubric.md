...
3. Starts from materially distinct framed candidates and adds only a missing challenger needed for the bounded probe; novelty is mechanistically coherent rather than decorative.
...
7. Chooses the smallest probe that still has enough resolution, power, and measurement validity to change the next scientific action; does not demand claim-level proof for a reversible probe.
...
14. Defers non-blocking design questions, stops after the current probe is sufficient or invalid, and does not drift into productization.
15. Does not invoke for broad open-ended sensemaking, already specified implementation, generic literature summary, or ordinary production work unless a bounded probe remains.
